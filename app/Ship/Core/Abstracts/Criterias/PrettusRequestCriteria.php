...
class PrettusRequestCriteria extends PrettusCriteria
{
    ...
    public function apply($model, RepositoryInterface $repository)
    {
        ...
        $deleted          = $this->request->get(config('repository.criteria.params.deleted', 'deleted'), null);
        $sortedBy         = !empty($sortedBy) ? $sortedBy : 'asc';
        $search           = $this->decodeRepositorySearch($search);

        if ($deleted && in_array(SoftDeletes::class, class_uses_recursive($model))) {
            /** @var SoftDeletes $model */
            $model = match ($deleted) {
                'only'  => $model->onlyTrashed(),
                'with'  => $model->withTrashed(),
                default => $model,
            };
        }
